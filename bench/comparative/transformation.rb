            ...
            module LiteBench
              def self.call(input)
                Validator
                  ...
                  .commit_at(:customer_name, from: %i[customer name]) { _1 }
                  .each_at(:line_items, from: [:items], commit: :array) { line_item(_1) }
                  .each_at(:charges, commit: :array) { charge(_1) }
                  .commit_at(:total, from: [%i[item_total charge_total]]) { _1.map(&:to_i).sum }
                  .auto_commit(as: :hash)
              end

              def self.line_item(item)
                item
                  .commit_at(:unit_price, from: %i[price]) { _1.to_i }
                  .commit_at(:name) { _1 }
                  .auto_commit(as: :hash)
              end
              ...
              def self.charge(charge)
                charge
                  .commit_at(:amount, from: %i[price]) { _1.to_i }
                  .commit_at(:name) { _1 }
                  .auto_commit(as: :hash)
              end
            end
